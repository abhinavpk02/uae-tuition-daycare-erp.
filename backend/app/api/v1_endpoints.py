...
async def create_student_v1(payload: StudentV1Create, db: AsyncSession = Depends(get_db)):
    try:
        ...
        # 2. Create Student
        prog_val = payload.program if isinstance(payload.program, StudentProgram) else StudentProgram(payload.program)
        student = Student(
...
async def create_staff_v1(payload: StaffV1Create, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if creator == "Admin" and payload.role in ["Admin", "SuperAdmin", UserRole.Admin, UserRole.SuperAdmin]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, 
